...
class TestSigilCompliantPipelineWithCrawl4AI:
    ...
    def local_llm_config(self):
        ...
        gcp_llm_endpoint = os.getenv(
            "GCP_LLM_ENDPOINT", "http://localhost:8000"
        )  # Your GCP endpoint
...
class TestCrawl4AIConfiguration:
    ...
    def test_rule_zero_alignment_principles(self):
        ...
        for principle, description in principles.items():
            assert (
                len(description) > 0
            ), f"Rule Zero principle {principle} must be documented"
...
if __name__ == "__main__":
    ...
    print(
        "3. Configure local LLM endpoint: export GCP_LLM_ENDPOINT=http://your-server:8000"
    )
    print("4. Enable LLM tests: export ENABLE_LLM_TESTS=1")
    print("\n🚀 Run tests:")
    print("   pytest tests/test_sigil_compliant_analyzer_real.py -v")
    print(
        "   pytest tests/test_sigil_compliant_analyzer_real.py::TestCrawl4AIConfiguration -v"
    )
    print("\n" + "=" * 60)
