...
pub fn add(a: i32, b: i32) -> i32 {
    a + b
        ...
        assert "prompt" in result
        assert "gen" in result
        assert "Adds two numbers" in result["gen"]
    ...
    x * y
        ...
        # May or may not find doc comment in code
        assert result is None or isinstance(result, dict)
...
pub fn factorial(n: u64) -> u64 {
    if n <= 1 { 1 } else { n * factorial(n - 1) }
        ...
        result = generate_explanation_task(code, None)
        # Should find doc comments in code
        assert result is None or isinstance(result, dict)
    ...
    def test_empty_doc_comment(self):
        ...
        # Empty doc should trigger synthesis
        assert result is None or isinstance(result, dict)
...
class TestLooksHelpers:
    ...
    def test_looks_explainable_with_doc(self):
        ...
        assert _looks_explainable("fn test() {}", "") is False
        assert _looks_explainable("fn test() {}", None) is False
