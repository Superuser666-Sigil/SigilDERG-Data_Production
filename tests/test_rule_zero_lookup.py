...
@pytest.mark.order(2)
def test_lookup_table_json_valid():
    """Lookup table is valid JSON and has required keys."""
    with open(LOOKUP_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    assert "environment_metadata" in data, "Missing environment_metadata in lookup table."
    assert isinstance(data["environment_metadata"], list), "environment_metadata should be a list."
    assert "rule_zero_policies" in data, "Missing rule_zero_policies in lookup table."
    assert isinstance(data["rule_zero_policies"], list), "rule_zero_policies should be a list."

@pytest.mark.order(3)
def test_environment_metadata_content():
    ...
    required_fields = ["label", "os_name", "os_version", "system_type", "enforcement_rank", "timestamp"]
    for field in required_fields:
        assert field in entry, f"Missing field in environment metadata: {field}"

@pytest.mark.order(4)
def test_rule_zero_policy_content():
    """If any policies exist, they must have required fields and valid JSON."""
    with open(LOOKUP_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    for policy in data["rule_zero_policies"]:
        for field in ["policy_name", "policy_type", "description", "policy_json", "enforcement_rank", "last_updated"]:
