...
def pytest_addoption(parser):
    ...
    parser.addoption("--coverage-focus", action="store_true", help="Run coverage tests only")
...
def pytest_collection_modifyitems(config, items):
    ...
    if config.getoption("--fast"):
        ...
        for item in items:
            if "slow" in item.keywords or any(name in str(item.fspath) for name in ["crawl4ai", "integration"]):
                item.add_marker(skip_slow)
    
    if config.getoption("--coverage-focus"):
        # Run only coverage tests
        coverage_files = ["test_config_coverage.py", "test_github_token_checker_coverage.py", "test_main_module_coverage.py", "test_rust_analyzer_coverage.py"]
...
def mock_config():
    ...
    return PipelineConfig(batch_size=2, n_workers=1, enable_crawl4ai=False, max_tokens=50)
